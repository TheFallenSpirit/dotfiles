...
const Player = (player) => {
    ...
    const Title = Widget.Label({
        ...
        className: 'title', label: player?.bind('track-title') || 'No Media Playing',
    });

    const Artists = Widget.Label({
        wrap: true, hpack: 'start',
        cursor: 'text', truncate: 'end', className: 'artists',
        label: player?.bind('track-artists').transform(a => a.join(', ')) || 'No Media Playing',
    });

    const Next = Widget.Button({ onClicked: () => player.next(), child: Widget.Icon({ icon: 'tabler-next' }) });
    const Previous = Widget.Button({ onClicked: () => player.previous(), child: Widget.Icon({ icon: 'tabler-previous' }) });
    ...
    const Toggle = Widget.Button({
        className: 'primary', onClicked: () => player.playPause(),
        child: Widget.Icon({ icon: player?.bind('play-back-status').transform(p => p === 'Playing' ? 'tabler-pause' : 'tabler-play') || 'tabler-play' }),
    });

    let Position = Widget.Label({ label: '0:00', hpack: 'start', className: 'position' })
    if (player !== null) Position = Widget.Label({ hpack: 'start', className: 'position' }).hook(player, self => {
    ...
    const Controls = Widget.CenterBox({
        ...
        endWidget: Widget.Label({ hpack: 'end', className: 'position', label: player?.bind('length').transform(lengthToStr) || '0:00' }),
        centerWidget: Widget.Box({ spacing: 4, children: [Previous, Toggle, Next] })
    });

    const Media = Widget.CenterBox({
        vertical: true, endWidget: Controls, className: 'media container',
...
export default () => Widget.Box({ className: 'media', children: players.transform(p => p.slice(0, 1).map(Player)) });
